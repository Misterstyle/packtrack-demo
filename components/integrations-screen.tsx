  ...
  useEffect(() => {
    ...
    if (scanPhaseIdx >= scanPhases.length - 1) {
      ...
      const timer = setTimeout(async () => {
        ...
        // Show success toast using Radix UI toast
        if (totalImported > 0) {
          toast({
            title: "Synchronisatie voltooid",
            description: `${totalImported} ${totalImported === 1 ? "pakket" : "pakketten"} succesvol geïmporteerd`,
            variant: "default",
          })
        } else if (skippedCount > 0) {
          toast({
            title: "Synchronisatie voltooid",
            description: "Alle pakketten waren al in je overzicht",
            variant: "default",
          })
        }
    ...
    return () => clearTimeout(timer)
  }, [scanning, scanPhaseIdx, addShipment, updateShipment, shipments, integrations, userId, refreshShipments, toast])
